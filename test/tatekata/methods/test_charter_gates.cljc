...
(def ^:private lexdir (java.io.File. actor-dir "schema/lex"))
...
(defn- required-union [doc]
  (let [acc (atom #{})]
    (letfn [(walk [x] (cond (map? x) (do (when (sequential? (or (get x "required") (:required x))) (swap! acc into (or (get x "required") (:required x)))) (doseq [v (vals x)] (walk v)))
