(ns tatekata.methods.social
  "social.cljc — 建方 — Construction DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tatekata's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "建方 — Construction が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over tatekata's own domain (Infrastructure construction robotics orchestration. Tier-B actor coordinating multi-robot site workflows (Giemon, Otete, Mimi), material…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tatekata R0: live social posting is Council Lv6+ + operator + "
