...
;; Schema lives at <git-root>/schema/tatekata.edn.
...
(defn schema-path [] (file-join root "schema" "tatekata.edn"))
...
(defn merge-schema! [new-attrs]
  (let [existing (load-schema)
    ...
    (spit (schema-path)
          (str ";; schema/tatekata.edn — Datomic/Datascript compatible (auto-generated)\n"
               (pr-str (vec merged)) "\n"))))
